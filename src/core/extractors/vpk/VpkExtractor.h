...
#ifdef QT_CORE_LIB
class VpkExtractor : public IExtractor
#else
class VpkExtractor
#endif
{
public:
...
#ifdef QT_CORE_LIB
    QString GetName() const override { return "VpkExtractor"; }
    QStringList GetSupportedExtensions() const override { return { "vpk" }; }
    ...
    bool ExtractSingle(const std::string& vpkPath, const std::string& outputDir);
    size_t ExtractMultiple(const std::vector<std::string>& vpkPaths,
