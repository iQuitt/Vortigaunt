...
static vlBool DecompressNativeDXT1(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight)
{
	...
	for (y = 0; y < uiHeight; y += 4)
	{
		for (x = 0; x < uiWidth; x += 4)
		{
			...
			{
				// Three-color block: derive the other color, 11 = transparent.
...
static vlBool DecompressNativeDXT3(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight)
{
	vlUInt		x, y, i, j, k, Select;
	vlByte		*Temp;
	Colour565	*color_0, *color_1;
...
static vlBool DecompressNativeDXT5(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight)
{
	vlUInt		x, y, i, j, k, Select;
	vlByte		*Temp;
	Colour565	*color_0, *color_1;
...
// Decodes a single 8-byte BC4-style alpha block into 16 values (used for ATI1N/ATI2N).
static vlVoid DecodeBC4Block(const vlByte *block, vlByte values[16])
{
	vlByte alphas[8];
	alphas[0] = block[0];
	alphas[1] = block[1];
	...
	vlUInt64 bits = 0;
	for (vlUInt i = 0; i < 6; i++)
		bits |= ((vlUInt64)block[2 + i]) << (8 * i);

	for (vlUInt i = 0; i < 16; i++)
		values[i] = alphas[(bits >> (3 * i)) & 0x07];
}

// ATI1N (BC4): single channel, replicated to RGB for display.
static vlBool DecompressNativeATI1N(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight)
{
	vlUInt iBps = 4 * uiWidth;
	vlByte *Temp = src;
	vlByte values[16];
		...
		for (vlUInt x = 0; x < uiWidth; x += 4)
		{
			...
			for (vlUInt j = 0; j < 4; j++)
			{
				for (vlUInt i = 0; i < 4; i++)
				{
					if (((x + i) < uiWidth) && ((y + j) < uiHeight))
					{
						vlUInt Offset = (y + j) * iBps + (x + i) * 4;
						vlByte v = values[j * 4 + i];
						dst[Offset + 0] = v;
						dst[Offset + 1] = v;
						dst[Offset + 2] = v;
						dst[Offset + 3] = 0xFF;
...
// ATI2N (BC5): two channels (X/Y of a normal map); Z is reconstructed for display.
static vlBool DecompressNativeATI2N(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight)
{
	vlUInt iBps = 4 * uiWidth;
	vlByte *Temp = src;
	vlByte xValues[16], yValues[16];

	for (vlUInt y = 0; y < uiHeight; y += 4)
	{
		for (vlUInt x = 0; x < uiWidth; x += 4)
		{
			DecodeBC4Block(Temp, xValues);
			DecodeBC4Block(Temp + 8, yValues);
			Temp += 16;
			...
			for (vlUInt j = 0; j < 4; j++)
			{
				for (vlUInt i = 0; i < 4; i++)
				{
					...
					{
						vlUInt Offset = (y + j) * iBps + (x + i) * 4;
						vlByte vx = xValues[j * 4 + i];
						vlByte vy = yValues[j * 4 + i];

						// Reconstruct Z from X/Y (normal map convention)
						vlSingle fx = ((vlSingle)vx / 255.0f) * 2.0f - 1.0f;
						vlSingle fy = ((vlSingle)vy / 255.0f) * 2.0f - 1.0f;
						vlSingle fz2 = 1.0f - fx * fx - fy * fy;
						vlSingle fz = fz2 > 0.0f ? sqrtf(fz2) : 0.0f;

						dst[Offset + 0] = vx;
						dst[Offset + 1] = vy;
						dst[Offset + 2] = (vlByte)((fz * 0.5f + 0.5f) * 255.0f);
						dst[Offset + 3] = 0xFF;
					}
				}
			}
...
//
// Converts data from the DXTn to RGBA8888 format. Data is read from *src
// and written to *dst. Width and height are needed to it knows how much data to process
//-----------------------------------------------------------------------------------------------------
vlBool CVTFFile::DecompressDXTn(vlByte *src, vlByte *dst, vlUInt uiWidth, vlUInt uiHeight, VTFImageFormat SourceFormat)
{
	...
	case IMAGE_FORMAT_DXT1:
	case IMAGE_FORMAT_DXT1_ONEBITALPHA:
		return DecompressNativeDXT1(src, dst, uiWidth, uiHeight);
	case IMAGE_FORMAT_DXT3:
		return DecompressNativeDXT3(src, dst, uiWidth, uiHeight);
	case IMAGE_FORMAT_DXT5:
		return DecompressNativeDXT5(src, dst, uiWidth, uiHeight);
	case IMAGE_FORMAT_ATI1N:
		return DecompressNativeATI1N(src, dst, uiWidth, uiHeight);
	case IMAGE_FORMAT_ATI2N:
		return DecompressNativeATI2N(src, dst, uiWidth, uiHeight);
	default:
		LastError.Set("Unsupported DXT format.");
		return vlFalse;
	}
}
...
{
	(void)lpSource; (void)lpDest; (void)uiWidth; (void)uiHeight; (void)DestFormat; (void)nAlphaThreshold;

	LastError.Set("DXT compression is not supported in this build of VTFLib.");
	return vlFalse;
}
